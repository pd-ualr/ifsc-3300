...
// prompt to ask the user about score
do {
    score = parseInt(
...
// what happens if the score is not -1
if (scores.length > 0) {
    ...
    // calculate and display the average and highest score
    const average = parseInt(total/scores.length);
    document.write(`<p>Average score is ${average}</p>`);
    document.write(`<p>Highest score is ${highest_score}</p>`);
}